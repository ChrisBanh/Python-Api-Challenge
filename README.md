...
The Python Api Challenge is divided into 2 components: WeatherPy and VacationPy. WeatherPy takes up the majority of this challenge as not only you would have to utilise an api to grab a list of cities with their respective weather and position information but also how it's position (especially the latitude) correlates to other meteorlogical conditions like humidity, cloudiness, wind speed and temperature. Scatter plots and linear regressions are also constructed from the dataframe created from the api. 

The second component, VacationPy, involves creating a map containing all of the cities within the df created from the WeatherPy component in accordance to Humidity. Ideal weather conditions (set by the user) are used to filter out locations that doesn't match all of the queries and closest hotels are then determined within a 10km radius for each city with ideal conditions. Finally these locations are then mapped out in a hvplot with all of the additional information including closest hotel with a 10km radius and the name of the hotel.  
...
#  Module 6 Challenge
Due Apr 24 by 9:59am Points 100 Submitting a text entry box or a website url
Background
